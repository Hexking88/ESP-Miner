...
void create_jobs_task(void *pvParameters)
{
    ...
    // Wacht tot de ASIC geïnitialiseerd is voordat we jobs gaan versturen
    ...
    while (1) {
        ...
        // Lees protocol dynamisch (coordinator kan gewisseld zijn)
        stratum_protocol_t active_protocol = GLOBAL_STATE->stratum_protocol;

        // Als protocol veranderd is, gooi huidig werk weg
        ...
        if (new_work != NULL) {
            ...
            // Protocol-switch tijdens dequeue?
            if (active_protocol != current_work_protocol) {
            ...
            // Bepaal clean_jobs flag van het nieuwe werk
            bool clean;
            if (active_protocol == STRATUM_PROTOCOL_V2) {
                if (stratum_v2_is_extended_channel(GLOBAL_STATE)) {
                    clean = ((sv2_ext_job_t *)new_work)->clean_jobs;
                } else {
                    clean = ((sv2_job_t *)new_work)->clean_jobs;
                }
            } else {
                clean = ((mining_notify *)new_work)->clean_jobs;
            }

            // Als het geen clean job is en we hebben al werk: negeer nieuwe job
            if (!clean && current_work != NULL) {
                ESP_LOGI(TAG, "Non-clean job ontvangen, negeren (behoud huidige werk)");
                free_work_item(GLOBAL_STATE, new_work, active_protocol);
                timeout_ms = ASIC_get_asic_job_frequency_ms(GLOBAL_STATE);
                continue;
            }

            // Vervang huidig werk (clean job, of we hadden nog niets)
            free_work_item(GLOBAL_STATE, current_work, current_work_protocol);
            ...
            // Voor SV2 non-extended: geen extranonce2 rolling, wacht op nieuw werk
        ...
        // Nogmaals protocol check (voor de zekerheid)
        active_protocol = GLOBAL_STATE->stratum_protocol;
        if (active_protocol != current_work_protocol) {
            free_work_item(GLOBAL_STATE, current_work, current_work_protocol);
            current_work = NULL;
